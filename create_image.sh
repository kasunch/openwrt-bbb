...
function create_bootscr() {
    ...
    local bootscr="${builder_dir}/staging_dir/target-arm_cortex-a8+vfpv3_musl_eabi/"`
                    `"image/ti_am335x-bone-black/boot.scr"

    mkimage -A arm -T script -c none -d "${boot_script}" "${bootscr}" > /dev/null || exit_on_error
}
...
packages="luci kmod-ath9k-htc hostapd-mini wpa-supplicant-mini "`
            `"kmod-usb-gadget-eth "`
